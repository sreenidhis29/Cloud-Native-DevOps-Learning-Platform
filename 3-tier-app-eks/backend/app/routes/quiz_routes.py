...
from app.models.models import Topic, Question, Leaderboard
...
def get_leaderboard():
    rankings = Leaderboard.query.order_by(Leaderboard.score.desc()).limit(10).all()
    return jsonify([r.to_dict() for r in rankings])

@quiz_bp.route('/leaderboard', methods=['POST'])
def add_to_leaderboard():
    data = request.get_json()
    if not all(k in data for k in ('username', 'score', 'mission_title')):
...
def submit_quiz():
    ...
    for question in questions:
        ...
        # Compare the submitted text with the correct option text
        correct_text = question.options[question.correct_answer]
        if str(submitted_answer).strip() == str(correct_text).strip():
            correct_count += 1
